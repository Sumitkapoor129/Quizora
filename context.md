...
6. **Phase 6 — Anti-cheat — DONE ✅** (session 2026-09-22, committed `2b22d8d` on `feature/anti-cheat` → merged to `main` as `9916e57`, branch deleted): 3-strike enforcement + 3rd-strike auto-submit, both sides.
...
7. **Phase 7 — Results — DONE ✅** (session 2026-09-22, committed `6abcf36` on `feature/results` → merge to `main`):
   - Server (`server/src/routes/admin.ts`): **`GET /api/admin/attempts`** — optional `?testId=` filter (blank/whitespace → unfiltered), newest-first (`_id:-1`; schema has no `createdAt`), **capped at 200** (`ponytail:` no cursor pagination yet). **`GET /api/admin/attempts/:attemptId`** — full **answer key derived from the sealed blueprint** (options in student-seen blueprint order, `options[].isCorrect` from `correctOptionIds`, `explanation`/text mapped from the live Test, deleted-test tolerant with key retained), sections breakdown, events incl. anti-cheat; `isAttempted` reflects live selections regardless of status (an admin watching a live attempt sees picks), correctness/marks gated on SUBMITTED/TIMED_OUT. Reused the student `buildContent` map pattern. `tests/admin-attempts.test.ts`: **11 tests** (403 ×2, empty list, enriched newest-first list, `?testId=` + blank filter, full answer-key detail SINGLE+MULTI+unattempted, **shuffled-options blueprint-order replay**, TIMED_OUT lazy-expiry, deleted-test serialization, 404).
   - Client: **`pages/student/Result.tsx`** real result view — **answer-key-free** (aggregate score, per-section + per-question correct/incorrect/unattempted badges + marks incl. negative, disclosure "Correct answers are not shown here"). **`pages/admin/attempts/{AttemptsList,AttemptDetail}.tsx`** — list table → detail with answer key (correct-option markers, "Your answer (wrong)" affordance, explanations, per-section breakdown, anti-cheat events grouped Violations/Activity vs informational), wired to `/admin/attempts` + new `/admin/attempts/:attemptId` (AppLayout). `api.admin.attempts.list/get`, admin attempt types (reuse `ResultSection`/`StudentAttemptEvent`), new `client/src/utils/format.ts` `formatDateTime`. Tests: **16** (Result ×4 incl. explicit no-leak assertion, AttemptsList ×4, AttemptDetail ×8).
   - Review: no Criticals. Fixed Important (unbounded list → `.limit(200)`; sealed-blueprint order previously untested → shuffled-options test) + Minors (blank `testId`; `isAttempted` no longer scored-gated; TIMED_OUT coverage added).
   - Verify: **server 106/106, client 54/54, both `tsc` builds clean**, bundle ~331KB (+13KB for 3 pages), zero new deps.
...
cd client && npm run build   # verified green every phase (latest: Phase 6)
```
