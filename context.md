...
5. **Phase 5 — Exam engine — DONE ✅** (session 2026-09-22, committed `2b4b07c` on `feature/exam-engine` → merge to `main`):
   ...
   - **Deferred (logged for Phase 6/9):** anti-cheat wiring is client-ready structure only (anti-cheat-list stubs exist); timer dead-center fullscreen auto-start + 3-strike logic pending; uploads GC + import size limits + rate-limit/pagination hardening remain for Phase 9; Result page still the Phase 3 stub (Phase 7).
6. **Phase 6 — Anti-cheat (NEXT):** fullscreen 3-warning (blocking modals), copy/paste/cut/context-menu suppression, visibility/focus events, event log, 3rd-strike auto-submit. Server endpoint + types already exist (`POST /api/student/attempts/:attemptId/events`, `AntiCheatEventType`, `PostEventResponse`); client listeners NOT yet wired in ExamRunner.
