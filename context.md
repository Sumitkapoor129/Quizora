...
7. **Phase 7 — Results (NEXT — start here).** Replace the Phase 3 stub `client/src/pages/student/Result.tsx` (currently a placeholder) with a real student summary (no answer key) + detailed admin result view. Starting points from existing work: `GET /api/student/attempts/:attemptId/result` (409 `NOT_SUBMITTED` while in progress; per-question marks exposed only after submit; no answer key/explanation — matches §3 DTO whitelist rule); `GET /api/admin/attempts[:id]` route exists (admin view can reuse it, keep exact-set/MULTI semantics from Phase 5 blueprint scoring); ExamRunner already navigates to the result route on SUBMITTED/TIMED_OUT/auto-submit; Phase 5 `TestAttempt` embeds `score` fields + per-section `sectionAttempts` + `answers[]` for the summary tables. Follow the same subagent workflow (UI → aesthetic → perf → code-quality → a11y), tests first, keep the student view answer-key-free.
